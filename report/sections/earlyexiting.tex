...
\begin{figure}
	...
	\caption[\gls{densenet}]{\gls{densenet}: The densely connected layers concatenate the output of all previous layers. The layers are grouped into blocks called dense blocks. Source: \citetitle{huang_densely_2016} \cite{huang_densely_2016}.}
	\label{fig:densenet}
\end{figure}
...
\begin{enumdescript}
	...
	\begin{enumdescript}
		...
		\begin{minipage}[t]{\linewidth}
			...
			\captionof{figure}[Cosine Annealing Learning Rate with Warm Restarts]{The cyclic learning rate of the using cosine annealing with warms restarts} 
		...
		\begin{minipage}[t]{\linewidth}
			...
			\captionof{figure}[Image Augmentaion Example]{Image Augmentation using \gls{imgaug}. The augmentation randomly applies noise injection, flipping, and affine transformations.}
...
In this section, we aim to evaluate the early exit capabilities of the different models in practice, where each exit has the same threshold. We experiment with raising the threshold for $ \gamma = \left\{0.1, 0.2, \dots 0,9\right\} $.  We exploit the exit mechanism to terminate inference if a satisfying score is obtained at an early exit. The last exit of the model classifies samples if it did not achieve a satisfying score on any of the early exits. Figure \ref{fig:model_c-threshold_comparison} presents the result using the score-max and figure \ref{fig:model_threshold_comparison} using the score-margin. The subfigures show the raised score requirements. Each subfigure presents a subplot of the frequency of correctly exited samples and a subplot of incorrectly exited samples at each exit for all three models. 
...
\begin{minipage}{\linewidth}
	...
	\begin{figure}
		...
		\caption[Model comparison of early exit capabilities using score-max]{Model max of early exit capabilities using score-max. The subfigures represent raising the threshold. Each subfigure contains a plot for correctly exited samples and another one for incorrectly exited samples at the different exits. }
...
\begin{figure}
	...
	\caption[Score-margin threshold and the Accuracy-Latency Trade-off]{Score-margin threshold used to control the accuracy-latency trade-off of the models on \protect\subref{fig:early_exit_vs_conv} GPU Workstation, \protect\subref{fig:jetson-early_exit_vs_conv} Jetson TX2, and \protect\subref{fig:nuc-early_exit_vs_conv} NUC }
...
\begin{figure}
	...
	\caption[Compact view of the Accuracy-Latency Trade-off]{The figure shows a compacted view of the accuracy-latency trade-off using the score-margin on \protect\subref{fig:early_exit_vs_conv} GPU Workstation, \protect\subref{fig:jetson-early_exit_vs_conv} Jetson TX2 and \protect\subref{fig:nuc-early_exit_vs_conv} NUC }
	\label{fig:threshold-acc-lat-trade-off-by-time}
\end{figure}

In figure \ref{fig:threshold-acc-lat-trade-off-by-time}, we show the accuracy-latency tradeoff by plotting accuracy against the meantime.
...
\begin{figure}
	...
	\hfill
	\subfloat[GPU Workstation\label{fig:delay-threshold-gpu}]{\includegraphics[width=\textwidth,height=.22\textheight,keepaspectratio]{figures/delay_plots/gpu__delay_threshold}}
	\hfill
	\subfloat[Jetson TX2\label{fig:delay-threshold-jetson}]{\includegraphics[width=\textwidth,height=.22\textheight,keepaspectratio]{figures/delay_plots/jetson__delay_threshold}}
	\hfill
	\subfloat[NUC\label{fig:delay-threshold-nuc}]{\includegraphics[width=\textwidth,height=.22\textheight,keepaspectratio]{figures/delay_plots/nuc__delay_threshold}}
	\caption[Reliability vs. Computation Latency]{Reliability vs. Delay Threshold on \protect\subref{fig:delay-threshold-gpu} GPU Workstation, \protect\subref{fig:delay-threshold-jetson} Jetson TX2, and \protect\subref{fig:delay-threshold-nuc} NUC}
