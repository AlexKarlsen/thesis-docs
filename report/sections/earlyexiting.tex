...
Assume $ K $ denotes the number of image classes, $ N $ denotes the number of the exit points in a DNN, $ I $ denotes the number of images.
	\begin{enumdescript}
		...
		\begin{enumdescript}
			...
			We denote a prediction vector $ \bm{p_{i}} $ containing binary values $ p_{i,n} $ indicating if the exit correctly predicted the sample or not.
			
			\begin{align}
				\bm{p_i} = \begin{bmatrix}
				\hat{y}^*_{i,1} == y_i \\
				\hat{y}^*_{i,2} == y_1 \\
				\hat{y}^*_{i,n} == y_i \\
				\vdots  \\
...
Still the challenge remains, how to determine when to let samples exit? It can be done using the output from the softmax classifier, which can be interpreted as a confidence measure for the prediction. In the next section we investigate how to allow samples to exit the model prematurely by a study of threshold metrics.
