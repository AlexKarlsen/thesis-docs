...
We still do not differentiate between early exit and conventional model,  as we use $ T_{i,n}^{cmp} $ to denote computation time irregardless of conventional inference model with a single exit, or early exit models with multiple. We do extend our view on latency, as we introduce two execution schemes, i.e.
	...
	\begin{enumdescript}
		...
		\begin{enumdescript}
			...
			\end{align*}
			\todo{should I denote it as sub $ T^{cmp}_n $? as there are downlink communication every time a prediction is acquired. Or split it in downlink and uplink?}
		...
		\begin{enumdescript}
			...
			\item[Latest] Under the assumption, that the last recieved prediction i.e. from deepest within the model has a better chance of being correct, as the exit has a higher accuracy than earlier exits. We define the method \emph{latest}, where we constrain ourselves to only use the most recent prediction $n$.
			\begin{align}
		...
		\item[Reliability]  We still define the reliability as in chapter \ref{ch:earlyexit}, as the fraction of samples, that can be correctly classified given a latency threshold $ \delta $. However, we use our new definition of accuracy.
		...
		\begin{align*}
			...
			\end{cases}
		\end{align*}
		We have a delay violation, if no prediction is provided before the deadline, i.e. $ T_{i,n} > \delta $  
	
		\item[Problem] As in chapter \ref{ch:earlyexit},  We want to maximize the reliability for each image by selecting the best possible exit
		\begin{maxi}
			{\vec{n}}{\bar{R}^f}
			{}{}
...
The score margin produces the worst results. Thus, having a big difference between the two highest scoring classes is not equivalent with being correct. Using the confidence max achieves close to the same performance as using latest for \gls{bresnet} and \gls{bdensenet}. The conclusion is, combining the information using these simple methods, is not able to improve the accuracy of the model. Our main goal is to improve accuracy, in situations where we cannot guarantee to have received all predictions. Henceforth we use the latest prediction, as this method gives the best results in general.  
...
Additionally, we made a measurement of the delay using the reliable transport protocol, \gls{tcp}. Figure \ref{fig:tcp-overhead} shows the impact of using \gls{tcp}. In the three different settings (meetingroom, kitchen, and corridor) the end-device is moved further away from the server, which reduce the quality of the communication link. In poor communication environment \gls{tcp} can be expected to have a high overhead caused by increasing retransmissions, thus having a negative impact on the overall delay.  
...
In \cite{kaya_shallow-deep_nodate} they do not operate with a time constraint and always have all predictions. Our goal was to improve the accuracy, when all predictions were not available, as we cannot be sure to have received all predictions within the time frame.  Our results show, that simply using the latest received predictions, is the best overall choice for these early exit models. 
...
Using \gls{aee} by offloading to the edge, we have managed to enable service at stringent delay thresholds not possible, when offloading conventional models for edge inference. We did successfully improve the reliability below 75 ms for the \gls{jetson}, and below 50 ms for the \gls{gpu-ws} using \gls{bresnet} and \gls{bdensenet}. Offloading the different models show overlapping reliability of the \gls{bresnet} and \gls{bdensenet}, hence it depends on the delay threshold which model to select.  
