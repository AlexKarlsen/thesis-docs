...
The aim is to find the metric, that reduces the amount of incorrectly exited samples ({\color{sns-red}red}). Whenever samples are exited incorrectly, overall accuracy of the models are reduced, if it could have been correctly classifed at later exit. The growing frequency of correctly exited samples ({\color{sns-green}green}) at later exits shows exactly this. Even though all samples will be classified at the last exit, hence no sample is in fact exited at this lat exit, thus it show the samples, that could not reach a confident score for the last exit. This means the accuracy for the last exit is not entirely correct. The end exit accuracy actually matches confidence threshold of 0.1 for all models, as no samples are not exited, see table \ref{tbl:earlyexitlastexit}.  

\begin{longtabu}{>{\bfseries}X|X}
