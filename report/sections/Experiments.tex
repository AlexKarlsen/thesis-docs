...
\section{Training Branchy-ResNet}

The model is trained on a single \gls{gpu} GTX1080 with 8Gb memory. Typically a batch size of 32 is chosen, however smaller batch size, have shown better generalization performance \cite{masters_revisiting_nodate}, thus a batch size of 16 is chosen, which also is maximum to prevent memory exhaustion and still have descent training times. Transfer learning are used  to reduce model convergence time and improve model accuracy \cite{yosinski_how_2014}. The network is initialized with weight pre-trained on the ImageNet dataset. The training progress is visualized in figure \ref{fig:B-resnet-miniimagenet-10}

\paragraph{MiniImagenet10}

The B-ResNet50 is trained on a subset of the ImageNet ILSVRC2012 dataset \cite{russakovsky_imagenet_2015} to reduce training time, but still being able to justify the BranchyNet framework. The subset contains all samples 10 randomly sampled classes, instead of the 1000 classes from the entire dataset. The dataset is inspired by MiniImageNet \cite{vinyals_matching_2016}, that uses a subset of 100 classes with 600 samples for each class.
