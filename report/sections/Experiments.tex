...
The aim is to find the metric, that reduces the amount of incorrectly exited samples (red). Whenever samples are exited incorrectly is reduces the models overall accuracy, if it could have been correctly classifed at later exit. A growing frequency of correctly exited samples (green) shows exactly this. Even though all samples will be classified at the last exit, hence no sample is in fact exited, thus it show the samples, that could not reach a confident score for the last exit. This means the accuracy for the last exit is not entirely correct in a test, where all samples are classified at the last exit, however this test was intended for finding the better metric not the exact accuracy for the models, this is the intention of another test \ref{label}.   
